			 ...
			 const std::size_t& s)
  {
	...
	if (server_)
      ...
      if (!aborted_) {
        ...
        startTimer();
        handleReadContent(err,responseBuf_.size());
    } else {
...
try {
    ...
    if(arg->is_http_scheme())
    {
        ...
        impl_.reset(new Client::SslImpl(*arg->ioService_, verifyEnabled_,
                    arg->server_,
                    context.context_, arg->sessionId_, arg->parsedUrl_.host));
#endif // WT_WITH_SSL
  } else {
    LOG_ERROR("unsupported protocol: " << arg->parsedUrl_.protocol);
    return false;
  }
  ...
  http_wire_encoder encoder(method,arg->parsedUrl_,message);
  encoder.stream_out_request(out);
  // start dns resolution
  impl_->request(methodNames_[method],
          arg->parsedUrl_.protocol,
          arg->parsedUrl_.auth,
          arg->parsedUrl_.host,
          arg->parsedUrl_.port,
          arg->parsedUrl_.path,
