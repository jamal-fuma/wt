...
namespace Wt
{
    ...
    namespace Chart
    {
        ...
        bool CurveLabel::checkIntersectHorizontal(const WPointF & p1, const WPointF & p2, double minX, double maxX, double y)
        {
            ...
            if(t <= 0 || t >= 1)
            {
                return false;    // The edge does not reach y, we do not consider the ends
            }
        ...
        class MarkerMatchIterator final : public SeriesIterator
        {
            public:
                ...
                const WDataSeries * matchedSeries() const
                {
                    return matchedSeries_;
                }
                int xRow() const
                {
                    return matchedXRow_;
        ...
        void WCartesianChart::setZoomAndPan()
        {
                    ...
                    axis(Axis::Y).zoomMax_ != WAxis::AUTO_MAXIMUM)
            {
                ...
                if(yZoom > axis(Axis::Y).maxZoom())
                {
                    yZoom = axis(Axis::Y).maxZoom();
                }
