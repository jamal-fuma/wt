...
void DatabaseImpl::Create(int dialect)
{
    ...
    {
        create.append(mServerName).append(":");
    }
    ...
    if(! mCreateParams.empty())
    {
        create.append(mCreateParams);
    }
...
void DatabaseImpl::Connect()
{
    ...
    if(! mRoleName.empty())
    {
        dpb.Insert(isc_dpb_sql_role_name, mRoleName.c_str());
    }
    if(! mCharSet.empty())
    {
        dpb.Insert(isc_dpb_lc_ctype, mCharSet.c_str());
    }
    std::string connect;
    if(! mServerName.empty())
    {
                        ...
                        bool * Sync, bool * Reserve)
{
    ...
    if(ODSMajor != 0)
    {
        *ODSMajor = result.GetValue(isc_info_ods_version);
    }
    if(ODSMinor != 0)
    {
        *ODSMinor = result.GetValue(isc_info_ods_minor_version);
    }
    if(PageSize != 0)
    {
        *PageSize = result.GetValue(isc_info_page_size);
    }
    if(Pages != 0)
    {
        *Pages = result.GetValue(isc_info_allocation);
    }
    if(Buffers != 0)
    {
        *Buffers = result.GetValue(isc_info_num_buffers);
    }
    if(Sweep != 0)
    {
        *Sweep = result.GetValue(isc_info_sweep_interval);
    }
    if(Sync != 0)
    {
...
void DatabaseImpl::Statistics(int * Fetches, int * Marks, int * Reads, int * Writes)
{
    ...
    if(Fetches != 0)
    {
        *Fetches = result.GetValue(isc_info_fetches);
    }
    if(Marks != 0)
    {
        *Marks = result.GetValue(isc_info_marks);
    }
    if(Reads != 0)
    {
        *Reads = result.GetValue(isc_info_reads);
    }
    if(Writes != 0)
    {
        *Writes = result.GetValue(isc_info_writes);
    }
}
                          ...
                          int * ReadIdx, int * ReadSeq)
{
    ...
    if(Update != 0)
    {
        *Update = result.GetCountValue(isc_info_update_count);
    }
    if(Delete != 0)
    {
        *Delete = result.GetCountValue(isc_info_delete_count);
    }
    if(ReadIdx != 0)
    {
        *ReadIdx = result.GetCountValue(isc_info_read_idx_count);
    }
    if(ReadSeq != 0)
    {
        *ReadSeq = result.GetCountValue(isc_info_read_seq_count);
    }
}
...
void DatabaseImpl::Users(std::vector<std::string> & users)
{
    ...
    while(*p == isc_info_user_names)
    {
        ...
        if(len != 0)
        {
            users.push_back(std::string().append(p, len));
        }
        p += len;   // Skip username
    }
    return;
