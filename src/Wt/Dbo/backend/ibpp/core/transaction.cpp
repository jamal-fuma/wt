...
void TransactionImpl::Rollback()
{
    if(mHandle == 0)
    {
        return;    // Transaction not started anyway
    }
    IBS status;
        ...
        IBPP::TAM am, IBPP::TIL il, IBPP::TLR lr, IBPP::TFF flags)
{
    ...
    if(am == IBPP::amRead)
    {
        tpb->Insert(isc_tpb_read);
    }
    else
    {
        tpb->Insert(isc_tpb_write);
    }
    switch(il)
    {
    ...
    {
        tpb->Insert(isc_tpb_wait);
    }
    if(flags & IBPP::tfIgnoreLimbo)
    {
        tpb->Insert(isc_tpb_ignore_limbo);
    }
    if(flags & IBPP::tfAutoCommit)
    {
        tpb->Insert(isc_tpb_autocommit);
    }
    if(flags & IBPP::tfNoAutoUndo)
    {
        tpb->Insert(isc_tpb_no_auto_undo);
    }
    mTPBs.push_back(tpb);
