...
int RowImpl::ColumnNum(const std::string & name)
{
    ...
    for(int i = 0; i < mDescrArea->sqld; i++)
    {
        ...
        if(var->sqlname_length != (int16_t)len)
        {
            continue;
        }
        if(strncmp(Uname, var->sqlname, len) == 0)
        {
            return i+1;
        }
    }
    ...
    {
        var = &(mDescrArea->sqlvar[i]);
        if(var->aliasname_length != (int16_t)len)
        {
            continue;
        }
        if(strncmp(Ualias, var->aliasname, len) == 0)
        {
            return i+1;
        }
    }
...
const char * RowImpl::ColumnName(int varnum)
{
    ...
    if(var->sqlname_length >= 31)
    {
        var->sqlname_length = 31;
    }
...
const char * RowImpl::ColumnAlias(int varnum)
{
    ...
    XSQLVAR * var = &(mDescrArea->sqlvar[varnum-1]);
    if(var->aliasname_length >= 31)
    {
        var->aliasname_length = 31;
    }
...
const char * RowImpl::ColumnTable(int varnum)
{
    ...
    XSQLVAR * var = &(mDescrArea->sqlvar[varnum-1]);
    if(var->relname_length >= 31)
    {
        var->relname_length = 31;
    }
...
IBPP::SDT RowImpl::ColumnType(int varnum)
{
    ...
    case SQL_VARYING :
        ...
        break;
    case SQL_SHORT :
        value = IBPP::sdSmallint;
        break;
    case SQL_LONG :
        value = IBPP::sdInteger;
        break;
    case SQL_INT64 :
        value = IBPP::sdLargeint;
        break;
    case SQL_FLOAT :
        value = IBPP::sdFloat;
        break;
    ...
    case SQL_ARRAY :
        ...
        break;
    default :
        throw LogicExceptionImpl("Row::ColumnType",
...
void RowImpl::SetValue(int varnum, IITYPE ivType, const void * value, int userlen)
{
    ...
    case SQL_TEXT :
        ...
        {
            if(userlen > var->sqllen)
            {
                userlen = var->sqllen;
            }
            memcpy(var->sqldata, value, userlen);
            while(userlen < var->sqllen)
            {
                var->sqldata[userlen++] = ' ';
            }
        }
    ...
    case SQL_DOUBLE :
        ...
        {
            *(double *)var->sqldata = *(double *)value;
        }
        break;
    case SQL_TIMESTAMP :
    ...
    if(var->sqltype & 1)
    {
        *var->sqlind = 0;    // Remove the 0 flag
    }
}
...
void * RowImpl::GetValue(int varnum, IITYPE ivType, void * retvalue)
{
    ...
    case SQL_VARYING :
        ...
        else if(ivType == ivByte)
        {
            ...
            if(retvalue != 0)
            {
                *(int *)retvalue = (int)*(int16_t *)var->sqldata;
            }
            value = var->sqldata+2;
        }
        else if(ivType == ivBool)
    ...
    if(mDescrArea != 0)
    {
        for(int i = 0; i < mDescrArea->sqln; i++)
        {
            ...
            if(var->sqldata != 0)
            {
                ...
                case SQL_ARRAY :
                case SQL_BLOB :
                    delete(ISC_QUAD *) var->sqldata;
                    break;
                case SQL_TIMESTAMP :
                    delete(ISC_TIMESTAMP *) var->sqldata;
                    break;
                case SQL_TYPE_TIME :
                    delete(ISC_TIME *) var->sqldata;
                    break;
                case SQL_TYPE_DATE :
                    delete(ISC_DATE *) var->sqldata;
                    break;
                case SQL_TEXT :
                case SQL_VARYING :
                    delete [] var->sqldata;
                    break;
                case SQL_SHORT :
                    delete(int16_t *) var->sqldata;
                    break;
                case SQL_LONG :
                    delete(int32_t *) var->sqldata;
                    break;
                case SQL_INT64 :
                    delete(int64_t *) var->sqldata;
                    break;
                case SQL_FLOAT :
                    delete(float *) var->sqldata;
                    break;
                case SQL_DOUBLE :
                    delete(double *) var->sqldata;
                    break;
                default :
                    throw LogicExceptionImpl("RowImpl::Free",
                                             _("Found an unknown sqltype !"));
                }
            }
            if(var->sqlind != 0)
...
void RowImpl::AllocVariables()
{
    ...
    for(i = 0; i < mDescrArea->sqld; i++)
    {
        ...
        case SQL_VARYING :
            ...
            break;
        case SQL_SHORT :
            var->sqldata = (char *) new int16_t(0);
            break;
        case SQL_LONG :
            var->sqldata = (char *) new int32_t(0);
            break;
        case SQL_INT64 :
            var->sqldata = (char *) new int64_t(0);
            break;
        case SQL_FLOAT :
        ...
        case SQL_DOUBLE :
            ...
            break;
        default :
            throw LogicExceptionImpl("RowImpl::AllocVariables",
                                     _("Found an unknown sqltype !"));
        }
        if(var->sqltype & 1)
        {
            var->sqlind = new short(-1);    // 0 indicator
        }
    }
}
...
RowImpl & RowImpl::operator=(const RowImpl & copied)
{
    ...
    for(int i = 0; i < mDescrArea->sqld; i++)
    {
        ...
        case SQL_VARYING :
            ...
            break;
        case SQL_SHORT :
            var->sqldata = (char *) new int16_t(*(int16_t *)org->sqldata);
            break;
        case SQL_LONG :
            var->sqldata = (char *) new int32_t(*(int32_t *)org->sqldata);
            break;
        case SQL_INT64 :
            var->sqldata = (char *) new int64_t(*(int64_t *)org->sqldata);
            break;
        case SQL_FLOAT :
            var->sqldata = (char *) new float(*(float *)org->sqldata);
            break;
        case SQL_DOUBLE :
            var->sqldata = (char *) new double(*(double *)org->sqldata);
            break;
        default :
            throw LogicExceptionImpl("RowImpl::Ctor",
                                     _("Found an unknown sqltype !"));
        }
        if(var->sqltype & 1)
        {
            var->sqlind = new short(*org->sqlind);    // 0 indicator
        }
    }
