    ...
    while(*p != isc_info_end)
    {
        ...
        else
        {
            ...
            case isc_spb_sec_username :
                ...
                if(len != 0)
                {
                    user.username.assign(p+3, len);
                }
                break;
            case isc_spb_sec_password :
                if(len != 0)
                {
                    user.password.assign(p+3, len);
                }
                break;
            case isc_spb_sec_firstname :
                if(len != 0)
                {
                    user.firstname.assign(p+3, len);
                }
                break;
            case isc_spb_sec_middlename :
                if(len != 0)
                {
                    user.middlename.assign(p+3, len);
                }
                break;
            case isc_spb_sec_lastname :
                if(len != 0)
                {
                    user.lastname.assign(p+3, len);
                }
                break;
            }
            p += (3 + len);
...
void ServiceImpl::GetUsers(std::vector<IBPP::User> & users)
{
    ...
    while(*p != isc_info_end)
    {
        ...
        else
        {
            ...
            case isc_spb_sec_username :
                ...
                if(! user.username.empty())
                {
                    users.push_back(user);    // Flush previous user
                }
                user.clear();
                if(len != 0)
                {
    ...
    if(! user.username.empty())
    {
        users.push_back(user);    // Flush last user
    }
}
                               ...
                               int pagesize, IBPP::BRF flags)
{
    ...
    unsigned int mask;
    if(flags & IBPP::brReplace)
    {
        mask = isc_spb_res_replace;
    }
    else
    {
        mask = isc_spb_res_create;    // Safe default mode
    }
