...
namespace Wt
{
    ...
    const char * WClientGLWidget::toString(WGLWidget::GLenum e)
    {
        ...
        case WGLWidget::UNPACK_FLIP_Y_WEBGL:
            return "ctx.UNPACK_FLIP_Y_WEBGL";
        case WGLWidget::UNPACK_PREMULTIPLY_ALPHA_WEBGL:
            return "ctx.UNPACK_PREMULTIPLY_ALPHA_WEBGL";
        case WGLWidget::CONTEXT_LOST_WEBGL:
            return "ctx.CONTEXT_LOST_WEBGL";
        case WGLWidget::UNPACK_COLORSPACE_CONVERSION_WEBGL:
            return "ctx.UNPACK_COLORSPACE_CONVERSION_WEBGL";
        case WGLWidget::BROWSER_DEFAULT_WEBGL:
    ...
    {
        js_ << "ctx.clear(";
        if(mask & WGLWidget::COLOR_BUFFER_BIT)
        {
            js_ << "ctx.COLOR_BUFFER_BIT|";
        }
        if(mask & WGLWidget::DEPTH_BUFFER_BIT)
        {
            js_ << "ctx.DEPTH_BUFFER_BIT|";
        }
        if(mask & WGLWidget::STENCIL_BUFFER_BIT)
        {
            js_ << "ctx.STENCIL_BUFFER_BIT|";
        }
        js_ << "0);";
        GLDEBUG;
    }
    ...
    {
        if((unsigned)buffer.getId() >= buffers_)
        {
            return;
        }
    ...
    {
        if((unsigned)buffer.getId() >= framebuffers_)
        {
            return;
        }
    ...
    {
        if((unsigned)buffer.getId() >= renderbuffers_)
        {
            return;
        }
    ...
    {
        if((unsigned)shader.getId() >= shaders_)
        {
            return;
        }
    ...
    {
        if((unsigned)texture.getId() >= textures_)
        {
            return;
        }
