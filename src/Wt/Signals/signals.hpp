        ...
        namespace Impl
        {
            ...
            struct ConnectHelper<0, Args...>
            {
                static connection connect(Signals::Signal<Args...> & signal,
                                          ...
                                          std::function<void ()> && f)
                {
                    return signal.connect([f WT_CXX14ONLY(=std::move(f))](Args...)
                    {
                        f();
                    }, target);
                }
            ...
            struct ConnectHelper<1, Args...>
            {
                ...
                {
                    return signal.connect([f WT_CXX14ONLY(=std::move(f))](B1 b1, An...)
                    {
                        f(b1);
                    }, target);
                }
            ...
            struct ConnectHelper<2, Args...>
            {
                ...
                {
                    return signal.connect([f WT_CXX14ONLY(=std::move(f))](B1 b1, B2 b2, An...)
                    {
                        f(b1, b2);
                    }, target);
                }
